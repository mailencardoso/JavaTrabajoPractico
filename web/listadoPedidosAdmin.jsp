...
<%}%>
 <div class="container-profile">
      <a type="submit" class="btn btn-secondary" id="seguir-comprando" href="listadoProductos.jsp">← Volver</a>
            ...
            <table class="table table-striped" id="tablaPedido">
                ...
                <tbody>
                   ...
                   <tr>
                     ...
                     <br>Mail: <%=pedidos.get(i).getMailCliente()%></td>
                     
                     <td>ID Pedido: <%=pedidos.get(i).getId()%>
                     <br>Fecha Pedido: <%=pedidos.get(i).getFechaPedido()%></td>
                     
                     <td><% for (int j=0;j<pedidos.get(i).getLineas().size();j++){%>
                         Nombre Producto: <%=pedidos.get(i).getLineas().get(j).getProducto().getNombre()%>
                     <br>Precio Unitario: <%=pedidos.get(i).getLineas().get(j).getProducto().getPrecio()%>
                     <br>Categoria: <%=pedidos.get(i).getLineas().get(j).getProducto().getCategoria()%>
                     <br>Cantidad: <%=pedidos.get(i).getLineas().get(j).getCantidad()%>
                     <%}%>
                     <br> Total <%pedidos.get(i).setPrecio();%>
